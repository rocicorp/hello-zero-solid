...
var require_main = __commonJS({
  "node_modules/dotenv/lib/main.js"(exports, module) {
    ...
    function _log(message2) {
      console.log(`[dotenv@${version}][INFO] ${message2}`);
    }
...
var secretKey = must(process.env.AUTH_SECRET, "required env var AUTH_SECRET");
...
var user = table("user").columns({
  id: string(),
  name: string(),
  partner: boolean()
}).primaryKey("id");
var medium = table("medium").columns({
  id: string(),
  name: string()
}).primaryKey("id");
var message = table("message").columns({
  id: string(),
...
var messageRelationships = relationships(message, ({ one }) => ({
  sender: one({
...
var builder = createBuilder(schema);

// shared/mutators.ts
function createMutators(userID) {
  return {
    message: {
      async create(tx, message2) {
        await tx.mutate.message.insert(message2);
      },
      async delete(tx, id) {
        mustBeLoggedIn(userID);
        await tx.mutate.message.delete({ id });
      },
      async update(tx, message2) {
        mustBeLoggedIn(userID);
        const prev = await tx.query.message.where("id", message2.id).one().run();
        if (!prev) {
          return;
        }
        ...
        await tx.mutate.message.update(message2);
      }
    }
  };
}
function mustBeLoggedIn(userID) {
  must(userID, "Must be logged in");
}

// server/mutate.ts
var processor = new PushProcessor(
  zeroPostgresJS(
    schema,
    postgres(
      must(
...
// shared/queries.ts
var queries = {
  users: syncedQuery("user", zod_default.tuple([]), () => builder.user),
  mediums: syncedQuery("medium", zod_default.tuple([]), () => builder.medium),
  ...
  filteredMessages: syncedQuery(
    ...
    zod_default.tuple([
      zod_default.object({
        senderID: zod_default.string(),
        mediumID: zod_default.string(),
        body: zod_default.string(),
        timestamp: zod_default.string()
      })
    ]),
    ({ senderID, mediumID, body, timestamp }) => {
      let q = builder.message.related("medium", (q2) => q2.one()).related("sender", (q2) => q2.one()).orderBy("timestamp", "desc");
      if (senderID) {
...
// server/get-queries.ts
var validated = Object.fromEntries(
